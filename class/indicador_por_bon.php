...
public function detalleGastos($fecha,$plaza,$almacen)
{
   ...
   $GROUP = "GROUP BY DESCRIPCION";
	 ...
	 $conn = conexion::conectar();
	 $res_array = array();
	 $sql = "SELECT AD_CXC_MOVTOS.IID_PLAZA,
										 PLAZA.V_RAZON_SOCIAL,
										 COUNT(AD_CXC_MOVTOS.IID_FOLIO) AS CANTIDAD_DEB
									  FROM AD_CXC_MOVTOS,
											AD_FA_FACTURA,
											CLIENTE,
											MONEDAS,
                      ...
                      AND E.SERIE(+) = 'K'
								 			AND ( AD_CXC_MOVTOS.N_TIPO_MOVTO = 3 )
								              AND  ( to_date(AD_CXC_MOVTOS.D_FECHA_MOVTO, 'dd/mm/yyy') >= (SELECT TRUNC(ADD_MONTHS(to_date('$fecha','YYYY-MM'), -3))FROM DUAL ))
                                AND  (to_date(AD_CXC_MOVTOS.D_FECHA_MOVTO, 'dd/mm/yyy') <= (SELECT  LAST_DAY(TO_DATE('$fecha', 'yyyy-mm'))FROM DUAL) )
								  			   AND CANCELADAS.IID_FOLIO IS NULL
                           AND AD_FA_FACTURA.IID_PLAZA IN ($andPlaza)
								  			   GROUP BY AD_CXC_MOVTOS.IID_PLAZA, PLAZA.V_RAZON_SOCIAL
								 			     ORDER BY AD_CXC_MOVTOS.IID_PLAZA";
						//5 HABILITADO 15 FISCAL HABILITADO
	 $stid = oci_parse($conn, $sql);
	 oci_execute($stid);
