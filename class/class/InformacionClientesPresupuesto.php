...
#echo $sql;
	...
	function tablaDetFacturado($pre,$fecha,$promotor,$plaza,$mes,$almacen)
	{
		...
		if ( $pre == 1){
			...
			if ( $plaza == "ALL" ){
				$andPlaza = " AND fac.iid_plaza in (2,3,4,5,6,7,8,17,18)";
			}else{
		...
		$sql_res1 = "( SELECT fac.iid_num_cliente, cli.v_razon_social AS cliente, al.iid_almacen, al.v_nombre AS almacen, pla.iid_plaza, pla.v_razon_social AS plaza,
									pro.iid_promotor, pro.iid_empleado, pro.v_nombre, pro.v_apellido_pat, pro.v_apellido_mat,
									to_char(mov.d_fecha_movto,'yyyy') AS anio ,to_char(mov.d_fecha_movto,'mm') as mes ,
				...
				INNER JOIN ad_fa_factura fac ON fac.iid_plaza = mov.iid_plaza AND fac.iid_folio = mov.iid_folio AND fac.iid_almacen <> 1336 AND fac.iid_almacen <> 1722
		...
		$sql_res2 = "(SELECT fac.iid_num_cliente, cli.v_razon_social AS cliente, al.iid_almacen, al.v_nombre AS almacen, pla.iid_plaza,
									pla.v_razon_social AS plaza, pro.iid_promotor, pro.iid_empleado, pro.v_nombre, pro.v_apellido_pat, pro.v_apellido_mat,  to_char(mov.d_fecha_movto,'yyyy') AS anio ,to_char(mov.d_fecha_movto,'mm') as mes ,
									NVL((NVL(sum(decode(fac.iid_moneda, 1, decode(fac.iva_ret, 0,
                                                               round(mov.n_monto_cargo / (1 + decode(fac.iva_tasa, .01, 0, fac.iva_tasa)), 2),
                                                               round(mov.n_monto_cargo / (1 + decode(fac.iva_tasa, .01, 0, fac.iva_tasa - .06 )), 2)),
                                                  2, decode(fac.iva_ret, 0,
			...
			$sql_res = " (SELECT fac.iid_num_cliente, cli.v_razon_social AS cliente, al.iid_almacen, al.v_nombre AS almacen, pla.iid_plaza, pla.v_razon_social AS plaza, pro.iid_promotor, pro.iid_empleado,
									pro.v_nombre, pro.v_apellido_pat, pro.v_apellido_mat,  to_char(mov.d_fecha_movto,'yyyy') AS anio ,to_char(mov.d_fecha_movto,'mm') as mes ,
									NVL((NVL(sum(decode(fac.iid_moneda, 1, decode(fac.iva_ret, 0,
                                                               round(mov.n_monto_cargo / (1 + decode(fac.iva_tasa, .01, 0, fac.iva_tasa)), 2),
                                                               round(mov.n_monto_cargo / (1 + decode(fac.iva_tasa, .01, 0, fac.iva_tasa - .06 )), 2)),
                                                  2, decode(fac.iva_ret, 0,
				...
				UNION
				(
				SELECT fac.iid_num_cliente, cli.v_razon_social AS cliente, al.iid_almacen, al.v_nombre AS almacen, pla.iid_plaza, pla.v_razon_social AS plaza, pro.iid_promotor, pro.iid_empleado,
							 pro.v_nombre, pro.v_apellido_pat, pro.v_apellido_mat,  to_char(mov.d_fecha_movto,'yyyy') AS anio ,to_char(mov.d_fecha_movto,'mm') as mes ,
							 NVL((NVL(sum(decode(fac.iid_moneda, 1, decode(fac.iva_ret, 0,
                                                                round(mov.n_monto_cargo / (1 + decode(fac.iva_tasa, .01, 0, fac.iva_tasa)), 2),
                                                                round(mov.n_monto_cargo / (1 + decode(fac.iva_tasa, .01, 0, fac.iva_tasa - .06 )), 2)),
	...
	function informacionAnualCliente($fecha,$promotor,$plaza, $almacen, $cliente){
		...
		$and_almacen = " ";
		if ($almacen != "ALL") {
			$and_almacen = " AND FAC.IID_ALMACEN = $almacen";
		}

		$andCliente = " ";
		if ($cliente != "ALL") {
			$andCliente = " AND FAC.IID_NUM_CLIENTE = $cliente";
		}
		...
		$sql = "SELECT *  FROM(
            SELECT dmes.nom_mes, 0 facturado_total, '' as cliente FROM vista_dashboard_mes_prospecto dmes
            UNION
            SELECT DECODE(to_char(mov.d_fecha_movto,'mm'),'01','Ene','02','Feb','03','Mar','04','Abr','05','May','06','Jun','07','Jul','08','Ago','09','Sep','10','Oct','11','Nov','12','Dic','Error') nom_mes ,
						NVL((NVL(sum(decode(fac.iid_moneda, 1, decode(fac.iva_ret, 0,
                                                               round(mov.n_monto_cargo / (1 + decode(fac.iva_tasa, .01, 0, fac.iva_tasa)), 2),
                                                               round(mov.n_monto_cargo / (1 + decode(fac.iva_tasa, .01, 0, fac.iva_tasa - .06 )), 2)),
                                                  2, decode(fac.iva_ret, 0,
            ...
            INNER JOIN plaza pla On pla.iid_plaza = fac.iid_plaza
            WHERE fac.status = 7 AND mov.n_status = 2 AND mov.n_tipo_movto IN (1,3,4)
            AND fac.iid_plaza IN ($andPlaza)
            AND to_char(mov.d_fecha_movto, 'yyyy') = $fecha
            AND fac.observacion <> 'GASTOS NOTARIALES POR NOTIFICACION DE REMATE'
            AND FAC.OBSERVACION <> 'VENTA DE 13,559 PIEZAS Y JUEGOS DE CALENTADORES SOLARES; EN EL ESTADO EN QUE SE ENCUENTRAN'
            $and_almacen
						$andCliente
            GROUP BY to_char(mov.d_fecha_movto,'mm'), cli.v_razon_social
          UNION
           SELECT DECODE(to_char(mov.d_fecha_movto,'mm'),'01','Ene','02','Feb','03','Mar','04','Abr','05','May','06','Jun','07','Jul','08','Ago','09','Sep','10','Oct','11','Nov','12','Dic','Error') nom_mes ,
					 NVL((NVL(sum(decode(fac.iid_moneda, 1, decode(fac.iva_ret, 0,
                                                               round(mov.n_monto_cargo / (1 + decode(fac.iva_tasa, .01, 0, fac.iva_tasa)), 2),
            ...
            $and_almacen
						$andCliente
            GROUP BY to_char(mov.d_fecha_movto,'mm'), cli.v_razon_social
					) x pivot((sum(facturado_total)) for nom_mes in ('Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic'))
					ORDER BY CLIENTE";
	...
	function clienteNombre($cliente){
		$conn = conexion::conectar();
		$res_array = array();
		$sql = "SELECT C.V_RAZON_SOCIAL  AS CLIENTE FROM CLIENTE C WHERE C.IID_NUM_CLIENTE = $cliente";
		$stid = oci_parse($conn, $sql);
		oci_execute($stid);
	...
	function informacionAnualCliente2($fecha,$promotor,$plaza, $almacen, $cliente){
	...
	///*******************************INFORMACION ANUAL CLIENTE************************************************///
	function informacionClientes($fecha,$promotor,$plaza, $almacen, $cliente){
		...
		$sql = "SELECT nom_mes, mes, SUM(facturado_total) facturado_total FROM
            (
            SELECT dmes.nom_mes, dmes.mes, 0 facturado_total FROM vista_dashboard_mes_prospecto dmes
            UNION
            SELECT DECODE(to_char(mov.d_fecha_movto,'mm'),'01','Ene','02','Feb','03','Mar','04','Abr','05','May','06','Jun','07','Jul','08','Ago','09','Sep','10','Oct','11','Nov','12','Dic','Error') nom_mes,
            to_char(mov.d_fecha_movto,'mm') as mes ,
						NVL((NVL(sum(decode(fac.iid_moneda, 1, decode(fac.iva_ret, 0,
                                                               round(mov.n_monto_cargo / (1 + decode(fac.iva_tasa, .01, 0, fac.iva_tasa)), 2),
                                                               round(mov.n_monto_cargo / (1 + decode(fac.iva_tasa, .01, 0, fac.iva_tasa - .06 )), 2)),
                                                  2, decode(fac.iva_ret, 0,
            ...
            FROM ad_cxc_movtos mov
            INNER JOIN ad_fa_factura fac ON fac.iid_plaza = mov.iid_plaza AND fac.iid_folio = mov.iid_folio AND fac.iid_almacen <> 43 AND fac.iid_almacen <> 1336 AND fac.iid_almacen <> 1722
            INNER JOIN cliente cli ON cli.iid_num_cliente = fac.iid_num_cliente
            INNER JOIN co_promotor pro ON pro.iid_promotor = cli.iid_promotor
            INNER JOIN co_promotor_fac_vs_pre vta ON vta.id_promotor = pro.iid_promotor/*add*/ AND to_char(mov.d_fecha_movto, 'yyyy') = vta.n_valor_anio
            INNER JOIN almacen al ON al.iid_almacen = fac.iid_almacen/*add*/
            WHERE fac.status = 7 AND mov.n_status = 2 AND mov.n_tipo_movto IN (1,3,4) AND vta.n_tipo = n_tipo
            AND to_char(mov.d_fecha_movto, 'yyyy') = $fecha
            AND fac.iid_plaza  IN ($andPlaza)
            $andCliente
            AND FAC.OBSERVACION <> 'VENTA DE 13,559 PIEZAS Y JUEGOS DE CALENTADORES SOLARES; EN EL ESTADO EN QUE SE ENCUENTRAN'
            AND al.s_tipo_almacen in (2,6) AND fac.observacion <> 'GASTOS NOTARIALES POR NOTIFICACION DE REMATE' /*add*/
            GROUP BY to_char(mov.d_fecha_movto,'mm')
          UNION
           SELECT DECODE(to_char(mov.d_fecha_movto,'mm'),'01','Ene','02','Feb','03','Mar','04','Abr','05','May','06','Jun','07','Jul','08','Ago','09','Sep','10','Oct','11','Nov','12','Dic','Error') nom_mes,
           to_char(mov.d_fecha_movto,'mm') as mes ,
					 NVL((NVL(sum(decode(fac.iid_moneda, 1, decode(fac.iva_ret, 0,
            ...
            AND fac.iid_plaza IN ($andPlaza)
            $andCliente
            AND pro.iid_promotor IN (173, 172,187,196,189,184,195)
            AND al.s_tipo_almacen in (2,6) AND fac.observacion <> 'GASTOS NOTARIALES POR NOTIFICACION DE REMATE' /*add*/
            AND FAC.OBSERVACION <> 'VENTA DE 13,559 PIEZAS Y JUEGOS DE CALENTADORES SOLARES; EN EL ESTADO EN QUE SE ENCUENTRAN'
