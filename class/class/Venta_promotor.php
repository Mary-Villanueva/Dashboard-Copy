...
class VentaPromotor
{
	...
	// ************************** VENTA REAL **************************  //
	function ventaReal($pre,$fecha,$promotor,$plaza)
	{
		$conn = conexion::conectar();
	...
	function tablaDetFacturado($pre,$fecha,$promotor,$plaza,$mes)
	{
	...
	function histMesConsol($pre,$fecha,$promotor,$plaza,$mes,$v_det)
	{
		...
		$stid = oci_parse($conn, "begin PCK_DASHBOARD.PCK_HIST_VTA_CONSOL(:n_tipo,:n_anio,:v_id_promotor,:n_id_plaza,:v_mes,:v_det,:cur_posicion); end;");
		oci_bind_by_name($stid, ':n_tipo', $pre);
		oci_bind_by_name($stid, ':n_anio', $fecha);
		oci_bind_by_name($stid, ':v_id_promotor', $promotor);
		oci_bind_by_name($stid, ':n_id_plaza', $plaza);
	...
	function histMesClieNew($pre,$fecha,$promotor,$plaza,$mes,$v_det)
	{
		...
		$stid = oci_parse($conn, "begin PCK_DASHBOARD.PCK_HIST_VTA_NEW_CLIE(:n_tipo,:n_anio,:v_id_promotor,:n_id_plaza,:v_mes,:v_det,:cur_posicion); end;");
		oci_bind_by_name($stid, ':n_tipo', $pre);
		oci_bind_by_name($stid, ':n_anio', $fecha);
		oci_bind_by_name($stid, ':v_id_promotor', $promotor);
		oci_bind_by_name($stid, ':n_id_plaza', $plaza);
