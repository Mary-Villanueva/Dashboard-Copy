...
class Op_in_car_des_info
{
		...
		if ($dia_manufac == true){
			switch (true) {
				case ( $fec_ini_per_manufac == true ) && ( $fec_fin_per_manufac == true ):
					...
					$and_slq_fecha_no_terminados = "  AND cargas.d_fec_recepcion < trunc(to_date('".$fec_ini_per_manufac."', 'dd-mm-yyyy'))  ";
					$and_sql_fecha_terminados = " AND TRUNC(cargas.d_fec_recepcion) < TO_DATE('".$fec_ini_per_manufac."', 'dd-mm-yyyy')
   																			AND TRUNC(cargas.d_fec_desp_vehic) = TO_DATE('".$fec_ini_per_manufac."', 'dd-mm-yyyy')";
					break;

				default:
					$and_sql_fecha_op_manufac = " AND TRUNC(cargas.d_fec_recepcion) = TO_DATE('".$dia_manufac."','dd-mm-yyyy') ";
					$and_slq_fecha_no_terminados = "    AND cargas.d_fec_recepcion < trunc(to_date('".$dia_manufac."', 'dd-mm-yyyy'))  ";
					$and_sql_fecha_terminados = " AND TRUNC(cargas.d_fec_recepcion) < TO_DATE('".$dia_manufac."', 'dd-mm-yyyy')
   																			AND TRUNC(cargas.d_fec_desp_vehic) = TO_DATE('".$dia_manufac."', 'dd-mm-yyyy')";
					break;
			}
		}
		/* CONCATENACION AND SQL */
		...
		if (!empty($plaza_manufac)) {
					if ($plaza_manufac == 'CÓRDOBA (ARGO)') {
						...
						$union_cordoba = $select_cd." FROM op_in_solicitud_carga_descarga".$data_link." cargas
								...
								AND cargas.n_status IN (1,2,3,4,5,6,9)
		...
		$conn = conexion::conectar();
	 	$sql_process = " UNION $select_cd
									 ...
									 WHERE cargas.id_tipo IN (1, 2)
									   ...
									   AND cargas.n_status IN (1, 2, 3, 4)
									   AND (cargas.n_virtual is null or cargas.n_virtual = 0)
   ...
   #
	...
	public function car_des_prog_info($plaza_manufac,$dia_manufac,$fec_ini_per_manufac,$fec_fin_per_manufac,$select_manufac_global_plaza)
	{
		...
		/* CONCATENACION AND SQL */
		$select_cd = " SELECT  cli.iid_num_cliente AS IID_NUM_CLIENTE, pla.iid_plaza AS id_plaza, pla.v_razon_social AS plaza, alm.v_nombre AS almacen, cargas.id_almacen AS almacen_id, cli.v_razon_social AS rs, cli.v_nombre_corto AS cliente, cargas.n_status AS status,
									to_char(cargas.d_fec_recepcion, 'dd-mm-yyyy HH24:MI:SS') AS registrado, to_char(cargas.d_fec_llegada_aprox, 'dd-mm-yyyy HH24:MI:SS') AS llega_aprox, to_char(cargas.d_fec_llegada_real, 'dd-mm-yyyy HH24:MI:SS') AS llega,
									to_char(cargas.d_fec_ini_car_des, 'dd-mm-yyyy HH24:MI:SS') AS inicia, to_char(cargas.d_fec_fin_car_des, 'dd-mm-yyyy HH24:MI:SS') AS fin,
					    		to_char(cargas.d_fec_desp_vehic, 'dd-mm-yyyy HH24:MI:SS') AS despacho,
									cargas.n_tiempo_operacion
					        ...
					        case when cli.iid_num_cliente = 2905 then ane.vid_factura else ' ' end as factura,
									cargas.v_nombre_chofer as chofer, cargas.v_identificacion as identificacion, cargas.v_transportes as linea_transporte";
		...
		if (!empty($plaza_manufac)) {
					if ($plaza_manufac == 'CÓRDOBA (ARGO)') {
						...
						$union_cordoba = $select_cd." FROM op_in_solicitud_carga_descarga".$data_link." cargas
								...
								LEFT JOIN plaza pla ON pla.iid_plaza = cargas.id_plaza
								LEFT JOIN cliente cli on cli.iid_num_cliente = cargas.id_cliente
								LEFT JOIN op_in_tipo_vehiculo veh on veh.iid_tipo = cargas.id_tipo_vehiculo
								LEFT JOIN co_ume ume on ume.iid_ume = cargas.iid_ume
								LEFT JOIN no_personal per on per.iid_empleado = cargas.id_almacenista
								LEFT JOIN OP_IN_ARRIBOS_NAD_ENC ane on ane.iid_arribo = cargas.iid_arr_ret
								WHERE cargas.id_tipo IN (1,2)
	...
	class Op_in_otros_info
	{
				...
				switch ($select_manufac_global_plaza[$i]){
							...
							//echo "3";
									...
									$union_cordoba = "SELECT alm.v_nombre AS almacen,
																	...
																	cli.iid_num_cliente as iid_num_cliente,
								...
								case '4':
									$data_link = " ";
									$inplaza = $inplaza. "4,";
									$union_Mex = "SELECT alm.v_nombre AS almacen,
																	pla.iid_plaza as id_plaza,
																	s_otros.id_almacen AS id_almacen,
																	s_otros.id_solicitud AS solicitud,
																	cli.v_razon_social AS rs_cli,
									...
									case '5':
										$data_link = " ";
										$inplaza = $inplaza. "5,";
										$union_Golfo = "SELECT alm.v_nombre AS almacen,
																		pla.iid_plaza as id_plaza,
																		s_otros.id_almacen AS id_almacen,
																		s_otros.id_solicitud AS solicitud,
																		cli.v_razon_social AS rs_cli,
										...
										case '6':
											$data_link = " ";
											$inplaza = $inplaza. "6,";
											$union_Peninsula = "SELECT alm.v_nombre AS almacen,
																			pla.iid_plaza as id_plaza,
																			s_otros.id_almacen AS id_almacen,
																			s_otros.id_solicitud AS solicitud,
																			cli.v_razon_social AS rs_cli,
											...
											case '7':
												$data_link = " ";
												$inplaza = $inplaza. "7,";
												$union_Puebla = "SELECT alm.v_nombre AS almacen,
																				pla.iid_plaza as id_plaza,
																				s_otros.id_almacen AS id_almacen,
																				s_otros.id_solicitud AS solicitud,
																				cli.v_razon_social AS rs_cli,
												...
												case '7':
	  											$data_link = " ";
	  											$inplaza = $inplaza. "8,";
	  											$union_Bajio = "SELECT alm.v_nombre AS almacen,
																					pla.iid_plaza as id_plaza,
																					s_otros.id_almacen AS id_almacen,
																					s_otros.id_solicitud AS solicitud,
																					cli.v_razon_social AS rs_cli,
	  											...
	  											break;
													...
													case '17':
		  											$data_link = " ";
		  											$inplaza = $inplaza. "17,";
		  											$union_Occidente = "SELECT alm.v_nombre AS almacen,
																						pla.iid_plaza as id_plaza,
																						s_otros.id_almacen AS id_almacen,
																						s_otros.id_solicitud AS solicitud,
																						cli.v_razon_social AS rs_cli,
		  											...
		  											break;
														...
														case '18':
			  											$data_link = " ";
			  											$inplaza = $inplaza. "18,";
			  											$union_Noreste = "SELECT alm.v_nombre AS almacen,
																							pla.iid_plaza as id_plaza,
																							s_otros.id_almacen AS id_almacen,
																							s_otros.id_solicitud AS solicitud,
																							cli.v_razon_social AS rs_cli,
		...
		// ------ INICIA METODO PARA CROSSDOCK DE VEHICULOS PROGRAMADOS ----------
		public function otros_info_car_des_prog($plaza_manufac,$dia_manufac,$fec_ini_per_manufac,$fec_fin_per_manufac,$select_manufac_global_plaza)
		{
	/* CONCATENACION AND SQL */
	if ($dia_manufac == true){
		switch (true) {
	...
	/* CONCATENACION AND SQL */
	$select_cd = " SELECT  CARGAS.ID_SOLICITUD AS Solicitud, cli.iid_num_cliente AS IID_NUM_CLIENTE, pla.iid_plaza AS id_plaza, pla.v_razon_social AS plaza, alm.v_nombre AS almacen, cargas.id_almacen AS almacen_id, cli.v_razon_social AS rs, cli.v_nombre_corto AS cliente, cargas.n_status AS status,
								to_char(cargas.d_fec_recepcion, 'dd-mm-yyyy HH24:MI:SS') AS registrado, to_char(cargas.d_fec_llegada_aprox, 'dd-mm-yyyy HH24:MI:SS') AS llega_aprox, to_char(cargas.d_fec_llegada_real, 'dd-mm-yyyy HH24:MI:SS') AS llega,
								to_char(cargas.d_fec_ini_car_des, 'dd-mm-yyyy HH24:MI:SS') AS inicia, to_char(cargas.d_fec_fin_car_des, 'dd-mm-yyyy HH24:MI:SS') AS fin,
								to_char(cargas.d_fec_desp_vehic, 'dd-mm-yyyy HH24:MI:SS') AS despacho,
								cargas.n_tiempo_operacion
...
#  echo $sql;
	...
	class Det_ret_arr
	{
		...
		public function info_det_arr_retEnc($plaza_manufac,$valor_almacen,$valor_arribo,$valor_arribo_descarga)
		{
			...
			$sql = "SELECT t.id_solicitud,
										 c.iid_num_cliente as iid_num_cliente,
							       C.V_RAZON_SOCIAL AS CLIENTE,
							       P.V_RAZON_SOCIAL,
							       A.V_NOMBRE,
							       t.d_fec_recepcion,
							       t.v_transportes,
							       t.v_nombre_chofer,
							       t.v_placas_vehiculo_real,
										 t.v_mercancia,
										 	to_char(t.d_fec_recepcion, 'dd-mm-yyyy HH24:MI:SS') AS registrado,
											to_char(t.d_fec_llegada_real, 'dd-mm-yyyy HH24:MI:SS') AS llega,
	 										to_char(t.d_fec_ini_car_des, 'dd-mm-yyyy HH24:MI:SS') AS inicia,
											to_char(t.d_fec_fin_car_des, 'dd-mm-yyyy HH24:MI:SS') AS fin,
	 					    			to_char(t.d_fec_desp_vehic, 'dd-mm-yyyy HH24:MI:SS') AS despacho,
											um.v_descripcion as UME_NOMBRE,
											t.n_cantidad_ume,
											t.n_tiempo_operacion,
							        enc.IID_RETIRO as iid_arr_ret,
											t.iid_regimen
							FROM OP_IN_SOLICITUD_CARGA_DESCARGA".$data_link." T
							INNER JOIN CLIENTE".$data_link." C ON T.ID_CLIENTE = C.IID_NUM_CLIENTE
							INNER JOIN PLAZA".$data_link." P ON T.ID_PLAZA = P.IID_PLAZA
							INNER JOIN ALMACEN".$data_link." A ON T.ID_ALMACEN = A.IID_ALMACEN
							INNER JOIN CO_UME UM ON  T.IID_UME = UM.IID_UME
							INNER JOIN OP_IN_RETIROS_NAD_ENC ENC ON T.ID_SOLICITUD = ENC.V_ID AND ROWNUM = 1
							INNER JOIN OP_IN_RETIROS_NAD_GRAL GRAL ON ENC.IID_ARCH_RETIRO = GRAL.IID_ARCH_RETIRO AND GRAL.IID_ALMACEN = A.IID_ALMACEN AND GRAL.IID_NUM_CLIENTE = T.ID_CLIENTE AND GRAL.IID_PLAZA = T.ID_PLAZA
										WHERE T.ID_SOLICITUD = $valor_arribo
							      AND A.IID_ALMACEN = $valor_almacen
							      AND T.ID_TIPO = 1
									 AND (T.n_virtual is null or T.n_virtual = 0)
							UNION
							SELECT t.id_solicitud,
														c.iid_num_cliente as iid_num_cliente,
											       C.V_RAZON_SOCIAL AS CLIENTE,
											       P.V_RAZON_SOCIAL,
											       A.V_NOMBRE,
											       ...
											       t.v_placas_vehiculo_real,
											        ...
											        enc.IID_RETIRO as iid_arr_ret,
															t.iid_regimen
											FROM OP_IN_SOLICITUD_CARGA_DESCARGA T
											INNER JOIN CLIENTE C ON T.ID_CLIENTE = C.IID_NUM_CLIENTE
											INNER JOIN PLAZA P ON T.ID_PLAZA = P.IID_PLAZA
											INNER JOIN ALMACEN A ON T.ID_ALMACEN = A.IID_ALMACEN
											INNER JOIN CO_UME UM ON  T.IID_UME = UM.IID_UME
											INNER JOIN OP_IN_RETIROS_NAD_ENC ENC ON T.ID_SOLICITUD = ENC.V_ID AND ROWNUM = 1
											INNER JOIN OP_IN_RETIROS_NAD_GRAL GRAL ON ENC.IID_ARCH_RETIRO = GRAL.IID_ARCH_RETIRO AND GRAL.IID_ALMACEN = A.IID_ALMACEN AND GRAL.IID_NUM_CLIENTE = T.ID_CLIENTE AND GRAL.IID_PLAZA = T.ID_PLAZA
														WHERE T.ID_SOLICITUD = $valor_arribo
			...
			$sql = "SELECT t.id_solicitud,
							...
							INNER JOIN ALMACEN".$data_link." A ON T.ID_ALMACEN = A.IID_ALMACEN
							INNER JOIN CO_UME UM ON  T.IID_UME = UM.IID_UME
										WHERE T.ID_SOLICITUD = $valor_arribo_descarga
							      AND A.IID_ALMACEN = $valor_almacen
							      AND T.ID_TIPO = 2
							UNION
							SELECT t.id_solicitud,
