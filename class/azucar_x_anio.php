...
public function grafica($anio,$n_semana){
		...
		$sql = "SELECT RH.N_MES, RH.MES, NVL((
       				SELECT SUM(S.TOTAL) FROM OP_IN_AZUCAR_TOTAL_MENSUAL S WHERE TO_CHAR(S.MES, '00') = to_char(RH.N_MES, '00') AND S.ANIO = '$anio' AND S.TIPO = 2), 0) AS VALOR
       				FROM RH_MESES_GRAFICAS RH
       				ORDER BY RH.N_MES";
				#echo $sql;
	...
	public function lastWeek(){
     ...
     FROM (select CASE WHEN (SUBSTR(t.vid_certificado, -1, 1) = 'N') OR T.VID_CERTIFICADO IS NULL THEN q.c_peso_total
           ...
           and q.iid_um = v.iid_ume
           and s.iid_num_cliente <> 1261
           and ((t.vid_certificado is not null AND t.vid_certificado NOT IN
						 (SELECT S.VID_CERTIFICADO FROM op_in_recibo_deposito S WHERE S.IID_ALMACEN  = 1468  AND S.VID_CERTIFICADO LIKE '%-N%' )))
           union all
           select CASE WHEN (SUBSTR(r.vid_certificado, -1, 1) = 'N') OR r.vid_certificado IS NULL THEN (q.c_peso_total * -1)
               END as CANTIDADN,
               CASE WHEN (SUBSTR(r.vid_certificado, -1, 1) = 'S') AND r.vid_certificado IS NOT NULL THEN (q.c_peso_total * -1)
...
public function graficaHabilitados($anio,$n_semana){
  		...
  		$sql = "SELECT RH.N_MES, RH.MES, (
       SELECT SUM(S.TOTAL) FROM OP_IN_AZUCAR_TOTAL_MENSUAL S WHERE TO_CHAR(S.MES, '00') = to_char(RH.N_MES, '00') AND S.ANIO = $anio AND S.TIPO = 1) AS VALOR
       FROM RH_MESES_GRAFICAS RH
       ORDER BY RH.N_MES";
  				#echo $sql;
  				$stid = oci_parse($conn, $sql);
...
public function graficaHabilitadoSemana($anio,$n_semana){
      		...
      		$sql = "SELECT T.NO_SEMANA, (SELECT Y.VALOR_TOTAL
                                       ...
                                       WHERE to_char(y.fecha_reg, 'yyyy') = '".$anio."'
																			    	 AND y.tipe = 2
                                             AND to_char(y.fecha_reg, 'IW') = t.no_semana) as valor
...
public function graficaDirectaSemana($anio,$n_semana){
				      		...
				      		$sql = "SELECT T.NO_SEMANA, NVL((SELECT Y.VALOR_TOTAL
				                                       ...
				                                       WHERE to_char(y.fecha_reg, 'yyyy') = '".$anio."'
																							    	 AND y.tipe = 1
				                                             AND to_char(y.fecha_reg, 'IW') = t.no_semana), 0) as valor
				                   FROM OP_NO_SEMANAS T
				                   ORDER BY T.NO_SEMANA";
				      				#echo $sql;
				      				$stid = oci_parse($conn, $sql);
...
public function ultimo_valor_mes($anio){
	$conn = conexion::conectar();
	$res_array = array();
	$anio = $anio-1;
	$sql = "SELECT SUM(TOTAL) AS TOTAL from op_in_azucar_total_mensual t where t.anio = $anio and t.mes = 12 ";
			$stid = oci_parse($conn, $sql);
			oci_execute($stid);
...
public function grafica_semana_ocupacion($anio,$n_semana){
				          ...
				          //$anio = $anio -1;
				      		/*$sql = "SELECT (
                                   ...
                                   INNER JOIN CLIENTE C ON C.IID_NUM_CLIENTE = T.IID_NUM_CLIENTE
                          ...
                          And c.iid_num_cliente <> 1261
                           and ((T.vid_certificado NOT IN (SELECT S.VID_CERTIFICADO FROM op_in_recibo_deposito S WHERE S.IID_ALMACEN = 1468 AND S.VID_CERTIFICADO LIKE '%-N%' )))
													) AS ENTRIES,
													Z.NO_SEMANA
													FROM OP_NO_SEMANAS Z";*/
											$sql = "SELECT (
													       	 SELECT --SUM(Q.C_PESO_TOTAL)/1000
                                   --A.IID_ALMACEN,
                                   SUM(Q.C_PESO_TOTAL)/1000
                                   FROM OP_IN_RECIBO_DEPOSITO T
													         INNER JOIN OP_IN_RECIBO_DEPOSITO_DET Q ON T.VID_RECIBO = Q.VID_RECIBO
													         INNER JOIN OP_IN_PARTES U ON U.VID_NUM_PARTE = Q.vid_num_parte
													         INNER JOIN CO_UME V ON Q.IID_UM = V.IID_UME
																	 INNER JOIN ALMACEN A ON T.IID_ALMACEN = A.IID_ALMACEN
                                   INNER JOIN CLIENTE C ON C.IID_NUM_CLIENTE = T.IID_NUM_CLIENTE
													WHERE  T.S_STATUS = 'P' AND U.V_PARTE_ALTERNATIVA like '%ZAFRA%'
                          AND TO_CHAR(T.D_PLAZO_DEP_INI, 'yyyy') = $anio
                          AND TO_CHAR(T.D_PLAZO_DEP_INI, 'IW') = Z.NO_SEMANA
                          And c.iid_num_cliente <> 1261
                          and ((T.vid_certificado NOT IN (SELECT S.VID_CERTIFICADO FROM op_in_recibo_deposito S WHERE S.IID_ALMACEN = 1468 AND S.VID_CERTIFICADO LIKE '%-N%' )))
...
public function inv_prom_semanal_zafra($anio,$n_semana){
				          ...
				          //$anio = $anio -1;
				      		...
				      		$sql = "SELECT ROUND(((SELECT SUM(Y.VALOR_TOTAL)
                                   ...
                                   AND to_char(y.fecha_reg, 'IW') <= t.no_semana
                                   AND TO_CHAR(Y.FECHA_REG, 'IW')>= 41))/ROWNUM, 2) as valor, T.NO_SEMANA, ROWNUM AS NUMERO_FILA
												            FROM OP_NO_SEMANAS T
												...
												SELECT ROUND(((SELECT SUM(Y.VALOR_TOTAL)
												                                   ...
												                                   WHERE to_char(y.fecha_reg, 'yyyy') =  '$anio'
												                                   AND to_char(y.fecha_reg, 'IW') <= t.no_semana
												                                   AND TO_CHAR(Y.FECHA_REG, 'IW')< 41)+
												         (SELECT SUM(Y.VALOR_TOTAL)
												                                   FROM op_meses_graficas_azucar y
												                                   WHERE to_char(y.fecha_reg, 'yyyy') =  '$anio2'
												                                   AND to_char(y.fecha_reg, 'IW') >= 41))/ (ROWNUM + 12),2) as valor, T.NO_SEMANA, ROWNUM + 12 AS NUMERO_FILA
												            FROM OP_NO_SEMANAS T
												            WHERE T.NO_SEMANA < 41
...
public function promedio_semanal_ejercicio($anio,$n_semana){
                ...
                WHERE to_char(y.fecha_reg, 'yyyy') = '$anio'
                  AND to_char(y.fecha_reg, 'IW') <= t.no_semana
                  AND TO_CHAR(Y.FECHA_REG, 'IW') < 41), 0) +
							             (SELECT SUM(Y.VALOR_TOTAL)
							                 FROM op_meses_graficas_azucar y
							                WHERE to_char(y.fecha_reg, 'yyyy') = '$anio2'
							                  AND to_char(y.fecha_reg, 'IW') >= 41)) / (ROWNUM + 12),
							             2) as valor,
							       T.NO_SEMANA,
...
public function histograma_lineal($anio,$n_semana){
				          ...
				          //$anio = $anio -1;
									$anio2 = $anio -1;
				      		$sql = "SELECT * FROM (SELECT SUM(T.TOTAL) AS VALOR_TOTAL,
       										TO_CHAR(to_date(TO_CHAR(T.MES, '00')|| '/' || T.ANIO, 'MM/YYYY'), 'MM/YYYY') AS FECHA
			 										FROM OP_IN_AZUCAR_TOTAL_MENSUAL T
													GROUP BY MES, T.ANIO ORDER BY ANIO, TO_NUMBER(MES, '9999'))
													UNION ALL
                ...
                WHERE to_char(y.fecha_reg, 'yyyy') = '2014'
	              ...
	              (SELECT SUM(Y.VALOR_TOTAL)
	                ...
	                WHERE to_char(y.fecha_reg, 'yyyy') = '2016'
	                  AND to_char(y.fecha_reg, 'IW') <= t.no_semana))/ 2,
	             2) as VALOR
  				 			FROM OP_NO_SEMANAS T
								ORDER BY T.NO_SEMANA ";
				      				#echo $sql;
...
public function media_historica2_2020($anio,$n_semana){
                 ...
                 FROM op_meses_graficas_azucar y
                WHERE to_char(y.fecha_reg, 'yyyy') = '$anio'
                  AND TO_CHAR(Y.FECHA_REG, 'IW') = 1), 0) +
							             (SELECT SUM(Y.VALOR_TOTAL)
							                 FROM op_meses_graficas_azucar y
							                WHERE to_char(y.fecha_reg, 'yyyy') = '$anio2'
							                  AND to_char(y.fecha_reg, 'IW') >= 41)),
