...
public function detalleGastos($fecha,$plaza,$almacen)
{
   ...
   $GROUP = "GROUP BY DESCRIPCION";
	 ...
	 $conn = conexion::conectar();
	 $res_array = array();
	 $sql = "SELECT P.V_RAZON_SOCIAL,
           DECODE(A.IID_ALMACEN , 1205, 'PEÑUELA',
                                  1208, 'PEÑUELA',
                                  1209, 'PEÑUELA',
                                  1210, 'PEÑUELA',
                                  1211, 'PEÑUELA',
           ...
           T.VIA_R_O,
           T.VIA_P_COTIZAR,
           T.POS_RACKS_VS_VIA_TRAD,
           T.POS_RACKS_VS_REAL,
           T.POS_RACKS_VS_VIA_COTIZAR,
           T.POS_RACKS_PROMEDIO,
           T.POS_RACKS_MINIMA,
           T.POS_RACKS_DESEABLE
            FROM OP_IN_VIAS_ALMACEN T
            INNER JOIN PLAZA P ON T.IID_PLAZA = P.IID_PLAZA
            INNER JOIN ALMACEN A ON A.IID_ALMACEN = T.IID_ALMACEN
            WHERE TO_DATE(N_ANIO || '-' || N_MES, 'YYYY-MM' ) = TO_DATE('$fecha', 'YYYY-MM')
            AND A.IID_PLAZA IN ($andPlaza)
            $andaid_almacen";
						//5 HABILITADO 15 FISCAL HABILITADO
	 $stid = oci_parse($conn, $sql);
	 oci_execute($stid);
