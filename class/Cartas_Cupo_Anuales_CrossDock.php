...
public function graficaNomina($fecha,$plaza)
    ...
    INNER JOIN op_in_recibo_deposito dep ON dep.vno_ped_imp = cupo.vno_pedimento
		INNER JOIN CLIENTE_CROSS_DOCK CD ON DEP.IID_NUM_CLIENTE = CD.IID_NUM_CLIENTE
...
public function grafica_Widgets_Mensual($fecha,$plaza){
			$andFecha = substr($fecha, 6,4);
			$fecha2 = $andFecha -1 ;
			$fecha3 = $fecha2 -1;
			#echo $andFecha;
			...
			$conn = conexion::conectar();
			$res_array = array();
			$sql = "SELECT COUNT(DISTINCT(D.TOTAL_CCE)) AS CCE_ANIO1,
       COUNT(D.total_cpe) AS CPE_ANIO1
  	 		FROM (SELECT pla.iid_plaza AS id_plaza,
               decode(to_char(pla.iid_plaza),
                      3,
                      'CÓRDOBA',
                      4,
                      ...
                      'OCCIDENTE',
                      18,
                      'NORESTE') as plaza_decode,
               pla.v_razon_social AS plaza,
               pla.v_siglas AS plaza_siglas,
               DECODE(cupo.v_status,
                      'RD',
                      case
                        when (to_char(cupo.d_fecha_arribo, 'yyyy') = '".$andFecha."') then
                         (dep.vno_ped_imp)
                      end) AS total_cce,
               DECODE(cupo.v_status,
                      'PE',
                      case
                        when (to_char(dep.d_plazo_dep_ini, 'yyyy') = '".$andFecha."') then
                         (cupo.iid_almacen)
                      end) AS total_cpe
          FROM plaza pla
          LEFT OUTER JOIN almacen alm ON alm.iid_plaza = pla.iid_plaza
          LEFT OUTER JOIN op_ce_cartas_cupo cupo ON cupo.iid_almacen =
                                                    alm.iid_almacen
         INNER JOIN op_in_recibo_deposito dep ON dep.vno_ped_imp =
                                                 cupo.vno_pedimento
          INNER JOIN CLIENTE_CROSS_DOCK CD ON DEP.IID_NUM_CLIENTE = CD.IID_NUM_CLIENTE
         WHERE pla.iid_plaza IN (".$in_plaza.")) D
 			 	ORDER BY D.ID_PLAZA";
										##echo $sql;
										 $stid = oci_parse($conn,$sql);
										 oci_execute($stid);
...
public function grafica_Widgets_Value($fecha,$plaza){
			$andFecha = substr($fecha, 6,4);
			$fecha2 = $andFecha -1 ;
			$fecha3 = $fecha2 -1;
			#echo $andFecha;
			...
			$conn = conexion::conectar();
			$res_array = array();
			$sql = "SELECT SUM(cupo.C_VAL_DOLARES * (SELECT R.C_TIPO_CAMBIO FROM OP_CE_TIPO_CAMBIO R WHERE R.DID_FECHA LIKE cupo.D_FECHA_EXPEDICION AND ROWNUM = 1)) AS VALUE_CARTAS
          FROM plaza pla
          LEFT OUTER JOIN almacen alm ON alm.iid_plaza = pla.iid_plaza
          LEFT OUTER JOIN op_ce_cartas_cupo cupo ON cupo.iid_almacen =
                                                    alm.iid_almacen
         INNER JOIN op_in_recibo_deposito dep ON dep.vno_ped_imp =
                                                 cupo.vno_pedimento
         INNER JOIN CLIENTE_CROSS_DOCK CD ON DEP.IID_NUM_CLIENTE =
                                             CD.IID_NUM_CLIENTE
         WHERE pla.iid_plaza IN (".$in_plaza.")
         AND (TO_CHAR(DEP.D_PLAZO_DEP_INI, 'yyyy') = ".$andFecha."
             OR TO_CHAR(CUPO.D_FECHA_ARRIBO, 'YYYY') = ".$andFecha.")
         AND( CUPO.V_STATUS = 'RD' OR CUPO.V_STATUS = 'PE')";
										#echo $sql;
										 $stid = oci_parse($conn,$sql);
										 oci_execute($stid);
