          ...
          function DetalleUbica(id_plaza,id_almacen,id_cliente,mercancia,rack,profundidad){
            ...
            $.ajax({
                ...
                var v_detalle = $.trim(dataJson[k].DETALLE1+" "+dataJson[k].DETALLE2);
                            ...
                            if ( v_detalle == "" ){
                              $("#imgUrl"+v_ubicacion).replaceWith('<a class="fancybox fancybox.iframe" href="rack_detubi.php?detUbi='+v_ubicacion+'&p=<?=$id_plaza?>&c=<?=$id_cliente?>&a=<?=$id_almacen?>&m=<?=$mercancia?>"><div class="v_libre itemMer palet" id="imgUrl'+v_ubicacion+'" data-info="libre '+(v_ubicacion).toLowerCase()+'" ></div></a>');
                              ...
                              $("#imgUrl"+v_ubicacion).replaceWith('<a class="fancybox fancybox.iframe" href="rack_detubi.php?detUbi='+v_ubicacion+'&p=<?=$id_plaza?>&c=<?=$id_cliente?>&a=<?=$id_almacen?>&m=<?=$mercancia?>"><div class="v_ocupado itemMer caja" id="imgUrl'+v_ubicacion+'" data-info="ocupado '+(v_detalle).toLowerCase()+'" ></div></a>');
...
<footer class="main-footer">
    ...
    <strong>Copyright &copy; 1991-<?=date("Y")?> <a href="http://www.argoalmacenadora.com.mx/index.php"> ARGO Almacenadora</a>.</strong> Todos los derechos reservados.
</footer>
...
function operacion(){
  ...
  var v_libre =  $('.v_libre').filter(function(){ return $(this).css('display') == 'block'; }).length,
  v_ocupado =  $('.v_ocupado').filter(function(){ return $(this).css('display') == 'block'; }).length,
  v_posicion = v_libre + v_ocupado,
  v_noubi =  $('.v_noubi').filter(function(){ return $(this).css('display') == 'block'; }).length;
