    ...
    <table class="table no-margin table-bordered table-striped">
      ...
      <tr>
       ...
       <table id="example" class="display nowrap" cellspacing="0" width="100%">
        ...
        <tbody>
        <?php
          $rackDetalleUbica = $objRack->rackDetalleUbica($ubicacion,$id_plaza,$fil_cliente,$id_almacen,$fil_db,$rtProyect,$tipe);
        ...
        ?>
            <tr>
                ...
                <td><?= $rackDetalleUbica[$i]["VID_NUM_PARTE"] ?></td>
                <td><?= $rackDetalleUbica[$i]["VID_NUM_PARTE_PADRE"] ?></td>
                <td><?= $rackDetalleUbica[$i]["V_DESCRIPCION"] ?></td>
                <td><?= $rackDetalleUbica[$i]["V_DESCRIPCION_GENERICA"] ?></td>
                <td><?= $rackDetalleUbica[$i]["NOMBRE_UME"] ?></td>

                <?php if ($rackDetalleUbica[$i]["I_SAL_CERO"] >= 1 ){ ?>
                <td bgcolor="#34D377"><?= $rackDetalleUbica[$i]["ENTRADA"] ?></td>
                <td bgcolor="#FFC95D"><?= $rackDetalleUbica[$i]["SALIDA"] ?></td>
                <td bgcolor="#33B6E5"><?= ($rackDetalleUbica[$i]["ENTRADA"] - $rackDetalleUbica[$i]["SALIDA"] )?></td>
                <?php }else{ ?>
                <td bgcolor="#34D377"><?= $rackDetalleUbica[$i]["ENTRADA"] ?></td>
                <td bgcolor="#FFC95D"><?= $rackDetalleUbica[$i]["SALIDA"] ?></td>
                <td bgcolor="#FF5D6C"><?= ($rackDetalleUbica[$i]["ENTRADA"] - $rackDetalleUbica[$i]["SALIDA"] )?></td>
                <?php } ?>
            </tr>
        <?php
  ...
  ?>
      ...
      <div class="direct-chat-msg">
       ...
       <table id="example" class="display nowrap" cellspacing="0" width="100%">
        ...
        <tbody>
        <?php
          $detMercanciaNoUbi = $objRack->detMercanciaNoUbi($arribo,$id_plaza,$fil_cliente,$id_almacen,$fil_db,$rtProyect,$tipe);

          for ($i=0; $i <count($detMercanciaNoUbi) ; $i++) {// FOR DETALLE MERCANCIA NO UBICADA
        ?>
            <tr>
                <td title="<?= $detMercanciaNoUbi[$i]["C_CLIENTE_CORTO"] ?>"><?= "(".$detMercanciaNoUbi[$i]["IID_NUM_CLIENTE"].")".$detMercanciaNoUbi[$i]["V_CLIENTE"] ?></td>
                <td><?= $detMercanciaNoUbi[$i]["V_UBICACION"] ?></td>
                <td><?php if ($detMercanciaNoUbi[$i]["N_LIBRE"] == 0){ echo 'OCUPADO';}else{echo 'LIBRE';} ?></td>
                <td><?= $detMercanciaNoUbi[$i]["ID_ARRIBO"] ?></td>
                <td><?= $detMercanciaNoUbi[$i]["V_CONTENEDOR"] ?></td>
                <td><?= $detMercanciaNoUbi[$i]["VID_RECIBO"] ?></td>
                <td><?= $detMercanciaNoUbi[$i]["VID_CERTIFICADO"] ?></td>
                <td><?= $detMercanciaNoUbi[$i]["VNO_PED_IMP"] ?></td>
                <td><?= $detMercanciaNoUbi[$i]["VID_NUM_PARTE"] ?></td>
                <td><?= $detMercanciaNoUbi[$i]["VID_NUM_PARTE_PADRE"] ?></td>
                <td><?= $detMercanciaNoUbi[$i]["V_DESCRIPCION"] ?></td>
                <td><?= $detMercanciaNoUbi[$i]["V_DESCRIPCION_GENERICA"] ?></td>
                <td><?= $detMercanciaNoUbi[$i]["NOMBRE_UME"] ?></td>

                <?php if ($detMercanciaNoUbi[$i]["I_SAL_CERO"] >= 1 ){ ?>
                <td bgcolor="#34D377"><?= $detMercanciaNoUbi[$i]["ENTRADA"] ?></td>
                <td bgcolor="#FFC95D"><?= $detMercanciaNoUbi[$i]["SALIDA"] ?></td>
                <td bgcolor="#33B6E5"><?= ($detMercanciaNoUbi[$i]["ENTRADA"] - $detMercanciaNoUbi[$i]["SALIDA"] )?></td>
                <?php }else{ ?>
                <td bgcolor="#34D377"><?= $detMercanciaNoUbi[$i]["ENTRADA"] ?></td>
                <td bgcolor="#FFC95D"><?= $detMercanciaNoUbi[$i]["SALIDA"] ?></td>
                <td bgcolor="#FF5D6C"><?= ($detMercanciaNoUbi[$i]["ENTRADA"] - $detMercanciaNoUbi[$i]["SALIDA"] )?></td>
                <?php } ?>
            </tr>
        <?php
