...
{
  header("location:azucar_x_anio.php");
  //return;
}
        ...
        <div class="row">
          ...
          <div class="box box-primary">
            <div class="box-header with-border">
              ...
              <a href="azucar_x_anio.php"><button class="btn btn-sm btn-warning">Borrar Filtros <i class="fa fa-close"></i></button></a>
              <?php } ?>
...
$(function () {
    ...
    #  echo  substr($hist_lineal[$i]["FECHA"], 0, 2);
      if($anio_actual == substr($hist_lineal[$i]["FECHA"], -4) AND $mes_actual == substr($hist_lineal[$i]["FECHA"], 0, 2)){
...
$(function () {
    ...
    var data2= [
      ...
      $ultimo_del_mes = $obj_class->ultimo_valor_mes($anio);
      if ($anio == $anio_actual) {
          $semana = date("W", strtotime($anio));
          $semana = $semana ;
      }else {
        ...
        for ($i=0; $i < $semana ; $i++) {
          if ($i == $semana-1 ) {
            ...
            if ($graficaDirSemana[$i]["VALOR"] == 0) {
                $valor_real2 = round($ultimo_del_mes[0]["TOTAL"]+$valorAcumulado);
            }else {
                $valor_real2 = round($ultimo_del_mes[0]["TOTAL"]+$valorAcumulado);
            }
            echo $valor_real2.",";
            ...
            $valorAcumulado += $grafica_ocupacion[$i]["ENTRIES"];
            $valor_real2 = $ultimo_del_mes[0]["TOTAL"]+$valorAcumulado;
            echo "$valor_real2 , ";
          }
    ...
    var data3= [
      ...
      $valor_weeek = $obj_class->lastWeek();
      $ultimo_del_mes = $obj_class->ultimo_valor_mes($ani2);
      if ($anio == $anio_actual) {
          $semana = date("W", strtotime($anio));
          $semana = $semana ;
      }else {
