...
$modulos_valida = Perfil::modulos_valida($_SESSION['iid_empleado'], '21');
if($modulos_valida == 0)
{
  header('Location: index.php');
}
...
//$pre,$fecha,$promotor,$plaza,$mes,$almacen
$diego = $insObj_VentaPromotor->tablaDetFacturado(3, 2021, 'ALL', 'ALL', '07', 'ALL');
?>
...
<!-- ########################################## Incia Contenido de la pagina ########################################## -->
 <div class="content-wrapper"><!-- Inicia etiqueta content-wrapper principal -->
    ...
    <section class="content-header">
      <h1>Dashboard <small>Ingresos</small></h1>
    </section>
    <!-- Main content -->
...
<?php if ($plaza <> 'ALL') { ?>
  $("#selectAlmacen").on("change",function (){
    location.href = "<?=basename($_SERVER['PHP_SELF']).'?'?>"+ "pre=<?=$pre?>&fecha="+<?=$fecha?>+"&promotor=<?=$promotor?>&plaza="+<?=$plaza?>+"&almacen="+$(this).val();
  });
<?php } ?>
  ...
  function graficaPreVsVta (meses) {
    ...
    $('#graficaPreVsVta').highcharts({
        chart: {
         ...
         title: {
          text: '<?php if( $pre == 3 ){ echo "PRESUPUESTO PLAZA $fecha VS RESULTADO PLAZA $fecha"; }else{echo "PRESUPUESTO PROMOTOR $fecha VS RESULTADO PROMOTOR $fecha";} ?>'
        },
        subtitle: {
