...
$modulos_valida = Perfil::modulos_valida($_SESSION['iid_empleado'], 52);
if($modulos_valida == 0)
{
  header('Location: index.php');
}
///////////////////////////////////////////
include '../class/Finiquitos_RH.php';
...
$tipo = "ALL";
if( isset($_GET["tipo"]) ){
  if( $_GET["tipo"] == "1" || $_GET["tipo"] == "2" || $_GET["tipo"] == "3" || $_GET["tipo"] == "4" || $_GET["tipo"] == "5" || $_GET["tipo"] == "6" || $_GET["tipo"] == "7" || $_GET["tipo"] == "8" || $_GET["tipo"] == "9" || $_GET["tipo"] == "10"  || $_GET["tipo"] == "11"){
    $tipo = $_GET["tipo"];
  }else{
    $tipo = "ALL";
  }
...
$graficaMensual2 = $modelNomina->grafica_Mensual2($fecha,$plaza,$tipo);
$graficaMensual3 = $modelNomina->grafica_Mensual3($fecha,$plaza,$tipo);
$graficaMensual4 = $modelNomina->grafica_Mensual4($fecha,$plaza,$tipo);
...
<div class="content-wrapper"><!-- Inicia etiqueta content-wrapper principal -->
  <section class="content-header">
    <h1>Dashboard<small>Finiquitos.</small></h1>
  </section>
  ...
  <section>
    <div class="box box-success">
      <div class="box-header with-border">
        <h3 class="box-title"><i class="fa fa-table"></i> DETALLES DE FINIQUITOS</h3>
        <div class="box-tools pull-right">
          <button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
          ...
          <table id="tabla_nomina" class="table table-striped table-bordered" cellspacing="0" width="100%">
            <thead>
              <tr>
                ...
                <th class="small" bgcolor="#383F6D"><font color="white">FECHA FINIQUITO</font></th>
                <th class="small" bgcolor="#383F6D"><font color="white">MOTIVO</font></th>
                <th class="small" bgcolor="#383F6D"><font color="white">MONTO</font></th>
              </tr>
            </thead>
            <tbody>
              ...
              <tr>
                ...
                <td class="small"><?= $tablaNomina[$i]["V_RAZON_SOCIAL"] ?></td>
                <td class="small"><?= $tablaNomina[$i]["D_FECHA_APLICACION"] ?></td>
                <td class="small"><?= $tablaNomina[$i]["OBSERVACION_DESPIDO"] ?></td>
                <td class="small"><?= $tablaNomina[$i]["C_MONTO"] ?></td>
  ...
  $(function(){
    ...
    var data1 = [
      ...
      for($i=0; $i< 5; $i++){
        ...
        echo $graficaGral[0]["TOTAL"].",";
     } ?>
    ];

    $('#grafica_general').highcharts({
      chart:{type: 'column'},
      title:{text: 'REPORTE GENERAL DE FINIQUITOS'},
      legend:{
        y:-40,
        borderWidth:1,
...
$(function () {
  ...
  var data1 = [
  <?php for ($i=0; $i <count($graficaNomina) ; $i++) {  ?>
  <?=$graficaNomina[$i]["EFECTIVO"]?>,
  <?php }  ?>
  ];

  $('#graficaNom').highcharts({
    chart: { type: 'column' },
    title: { text: 'FINIQUITOS GENERAL POR PLAZA'},
    legend:{
            y: -40,
            borderWidth: 1,
            backgroundColor: (Highcharts.theme && Highcharts.theme.legendBackgroundColor) || '#c6c2c2'
...
<script type="text/javascript">
  $(function(){
    ...
    var data1 = [
      <?php for($i=0; $i<count($graficaMensual); $i++){ ?>
        <?=$graficaMensual[$i]["EFECTIVO"]?>,
      <?php } ?>
    ];

    var data2 = [
      <?php for($i=0; $i<count($graficaMensual2); $i++){ ?>
        <?=$graficaMensual2[$i]["EFECTIVO"]?>,
      <?php } ?>
    ];

    var data3 = [
      <?php for($i=0; $i<count($graficaMensual3); $i++){ ?>
        <?=$graficaMensual3[$i]["EFECTIVO"]?>,
      <?php } ?>
    ];

    var data4 = [
      <?php for($i=0; $i<count($graficaMensual4); $i++){ ?>
        <?=$graficaMensual4[$i]["EFECTIVO"]?>,
      <?php } ?>
    ];
    ...
    $('#graficaMensual').highcharts({
      ...
      colors:['#33FA07', '#C21313', '#060606', '#FFC300'],
      plotOptions:{
        series:{
          minPointLength:3
        }
      ...
      series:[{
        name: 'Total Año <?php echo substr($fecha, 6,4); ?>',
        data: data1,
      },{
      name: 'Total Año <?php echo substr($fecha, 6,4)-1; ?>',
      data: data2,
      },{
      name: 'Total Año <?php echo substr($fecha, 6,4)-2; ?>',
      data: data3,
      },{
      name: 'Total Año <?php echo substr($fecha, 6,4)-3; ?>',
      data: data4,}]
    });
  });
</script>
