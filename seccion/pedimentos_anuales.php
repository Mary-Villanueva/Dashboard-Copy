...
{
  header("location:pedimentos_anuales.php");
  //return;
}
session_start();
...
$modulos_valida = Perfil::modulos_valida($_SESSION['iid_empleado'], 24);
if($modulos_valida == 0)
{
  header('Location: index.php');
}
///////////////////////////////////////////
include '../class/Pedimentos_Anuales.php';
...
//$tablaNomina = $modelNomina->grafica_Mensual1($fecha,$plaza);
...
<div class="content-wrapper"><!-- Inicia etiqueta content-wrapper principal -->
  <section class="content-header">
    <h1>Dashboard<small>Pedimentos Anuales</small></h1>
  </section>
  ...
  <section>
    <div class="box box-success">
      <div class="box-header with-border">
        <h3 class="box-title"><i class="fa fa-table"></i> Pedimentos Por Mes</h3>
        <div class="box-tools pull-right">
          <button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
          ...
          <table id="tabla_nomina" class="table table-striped table-bordered" cellspacing="0" width="100%">
            ...
            <tbody>
              ...
              <tr>
                ...
                <?php if ($i < 1){ ?>
                    <td class="small"><?= $graficaMensual[0]["TOTAL_PED_ANO1"] ?></td>
                    <td class="small"><?= $graficaMensual[1]["TOTAL_PED_ANO1"] ?></td>
                    <td class="small"><?= $graficaMensual[2]["TOTAL_PED_ANO1"] ?></td>
                    <td class="small"><?= $graficaMensual[3]["TOTAL_PED_ANO1"] ?></td>
                    <td class="small"><?= $graficaMensual[4]["TOTAL_PED_ANO1"] ?></td>
                    <td class="small"><?= $graficaMensual[5]["TOTAL_PED_ANO1"] ?></td>
                    <td class="small"><?= $graficaMensual[6]["TOTAL_PED_ANO1"] ?></td>
                    <td class="small"><?= $graficaMensual[7]["TOTAL_PED_ANO1"] ?></td>
                    <td class="small"><?= $graficaMensual[8]["TOTAL_PED_ANO1"] ?></td>
                    <td class="small"><?= $graficaMensual[9]["TOTAL_PED_ANO1"] ?></td>
                    <td class="small"><?= $graficaMensual[10]["TOTAL_PED_ANO1"] ?></td>
                    <td class="small"><?= $graficaMensual[11]["TOTAL_PED_ANO1"] ?></td>
                <?php }elseif ($i < 2) {?>
                  <td class="small"><?= $graficaMensual[0]["TOTAL_PED_ANO2"] ?></td>
                  <td class="small"><?= $graficaMensual[1]["TOTAL_PED_ANO2"] ?></td>
                  <td class="small"><?= $graficaMensual[2]["TOTAL_PED_ANO2"] ?></td>
                  <td class="small"><?= $graficaMensual[3]["TOTAL_PED_ANO2"] ?></td>
                  <td class="small"><?= $graficaMensual[4]["TOTAL_PED_ANO2"] ?></td>
                  <td class="small"><?= $graficaMensual[5]["TOTAL_PED_ANO2"] ?></td>
                  <td class="small"><?= $graficaMensual[6]["TOTAL_PED_ANO2"] ?></td>
                  <td class="small"><?= $graficaMensual[7]["TOTAL_PED_ANO2"] ?></td>
                  <td class="small"><?= $graficaMensual[8]["TOTAL_PED_ANO2"] ?></td>
                  <td class="small"><?= $graficaMensual[9]["TOTAL_PED_ANO2"] ?></td>
                  <td class="small"><?= $graficaMensual[10]["TOTAL_PED_ANO2"] ?></td>
                  <td class="small"><?= $graficaMensual[11]["TOTAL_PED_ANO2"] ?></td>
              <?php  }elseif ($i < 3) {?>
                <td class="small"><?= $graficaMensual[0]["TOTAL_PED_ANO3"] ?></td>
                <td class="small"><?= $graficaMensual[1]["TOTAL_PED_ANO3"] ?></td>
                <td class="small"><?= $graficaMensual[2]["TOTAL_PED_ANO3"] ?></td>
                <td class="small"><?= $graficaMensual[3]["TOTAL_PED_ANO3"] ?></td>
                <td class="small"><?= $graficaMensual[4]["TOTAL_PED_ANO3"] ?></td>
                <td class="small"><?= $graficaMensual[5]["TOTAL_PED_ANO3"] ?></td>
                <td class="small"><?= $graficaMensual[6]["TOTAL_PED_ANO3"] ?></td>
                <td class="small"><?= $graficaMensual[7]["TOTAL_PED_ANO3"] ?></td>
                <td class="small"><?= $graficaMensual[8]["TOTAL_PED_ANO3"] ?></td>
                <td class="small"><?= $graficaMensual[9]["TOTAL_PED_ANO3"] ?></td>
                <td class="small"><?= $graficaMensual[10]["TOTAL_PED_ANO3"] ?></td>
                <td class="small"><?= $graficaMensual[11]["TOTAL_PED_ANO3"] ?></td>
              <?php }?>
              </tr>
              <?php } ?>
            </tbody>
...
$(function () {
  ...
  var data1 = [
  <?php for ($i=0; $i <count($graficaNomina) ; $i++) {  ?>
  <?=$graficaNomina[$i]["TOTAL_PED_ANO1"]?>,
  <?php }  ?>
  ];

  var data2 = [
  <?php for ($i=0; $i <count($graficaNomina) ; $i++) {  ?>
  <?=$graficaNomina[$i]["TOTAL_PED_ANO2"]?>,
  <?php }  ?>
  ];

  var data3 = [
  <?php for ($i=0; $i <count($graficaNomina) ; $i++) {  ?>
  <?=$graficaNomina[$i]["TOTAL_PED_ANO3"]?>,
  <?php }  ?>
  ];

  $('#graficaNom').highcharts({
    chart: { type: 'column' },
    title: { text: 'Pedimentos de Extracción por Plaza' },
    legend:{
            y: -40,
            borderWidth: 1,
...
<script type="text/javascript">
  $(function(){
    ...
    var data1 = [
      <?php for($i=0; $i<count($graficaMensual); $i++){ ?>
        <?=$graficaMensual[$i]["TOTAL_PED_ANO1"]?>,
      <?php } ?>
    ];

    var data2 = [
      <?php for($i=0; $i<count($graficaMensual); $i++){ ?>
        <?=$graficaMensual[$i]["TOTAL_PED_ANO2"]?>,
      <?php } ?>
    ];

    var data3 = [
      <?php for($i=0; $i<count($graficaMensual); $i++){ ?>
        <?=$graficaMensual[$i]["TOTAL_PED_ANO3"]?>,
      <?php } ?>
    ];

    $('#graficaMensual').highcharts({
      chart:{type: 'line'},
      title:{text: 'Comparativo de Extracciones por Mes <?php if($plaza == 'ALL'){echo "";}else{echo $plaza;}?>'},
      legend:{
        y:-40,
        borderWidth:1,
        backgroundColor: (Highcharts.theme && Highcharts.theme.legendBackgroundColor) || '#c6c2c2'
